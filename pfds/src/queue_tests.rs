...
use crate::{ListQueue, OptimizedQueue, Queue, QueueError};
    ...
    A: Clone + PartialEq + std::fmt::Debug,
{
    let queue: Q = Q::empty();
    assert!(
        rust_fp_categories::Empty::is_empty(&queue),
        "空のキューはis_emptyがtrueを返すべき"
    );
    ...
    assert!(
        dequeue_result.is_err(),
        "空のキューからのdequeueはエラーを返すべき"
    );

    if let Err(err) = dequeue_result {
        assert!(
            matches!(err, QueueError::EmptyQueueError),
            "エラーの種類はEmptyQueueErrorであるべき"
        );
    }
}
    ...
    A: Clone + PartialEq + std::fmt::Debug + Default,
{
    ...
    assert_eq!(
        queue.size(),
        3,
        "3つの要素を追加した後のサイズは3であるべき"
    );
    assert!(
        !rust_fp_categories::Empty::is_empty(&queue),
        "要素を追加した後はis_emptyがfalseを返すべき"
    );
    ...
    assert_eq!(
        value, b,
        "2番目にdequeueされる値は2番目に追加された値であるべき"
    );
    ...
    assert_eq!(
        value, c,
        "3番目にdequeueされる値は3番目に追加された値であるべき"
    );
    assert_eq!(
        queue.size(),
        0,
        "すべての要素をdequeueした後のサイズは0であるべき"
    );
    assert!(
        rust_fp_categories::Empty::is_empty(&queue),
        "すべての要素をdequeueした後はis_emptyがtrueを返すべき"
    );
    ...
    A: Clone + PartialEq + std::fmt::Debug + Default,
{
    ...
    assert_eq!(
        queue.size(),
        expected_items.len(),
        "from_iterで作成されたキューのサイズは元のコレクションのサイズと同じであるべき"
    );
    ...
    assert_eq!(
        queue_items, expected_items,
        "キューから取り出した要素は元のコレクションと同じ順序であるべき"
    );
}
    ...
    Q: Queue<i32> + Empty,
{
    ...
    assert_eq!(
        queue.size(),
        element_count,
        "大量の要素を追加した後のサイズは追加した要素数と同じであるべき"
    );
    ...
    Q: Queue<i32> + Empty,
{
    ...
    for i in 0..100 {
        ...
        assert_eq!(
            value, i,
            "交互操作時にdequeueされた値は直前にenqueueされた値と同じであるべき"
        );
        queue = new_queue;
    }
...
mod list_queue_tests {
    ...
    fn test_immutability() {
        ...
        assert_eq!(
            queue2.size(),
            2,
            "クローンされたキューは元のキューの操作の影響を受けないべき"
        );
        assert_eq!(
            queue2.peek().unwrap(),
            1,
            "クローンされたキューの先頭要素は変わらないべき"
        );
        ...
        assert_eq!(
            queue1.size(),
            2,
            "元のキューはdequeue操作後も変更されないべき"
        );
        assert_eq!(
            queue1.peek().unwrap(),
            1,
            "元のキューの先頭要素はdequeue操作後も変更されないべき"
        );

        // dequeue後の新しいキューは変更されていることを確認
        assert_eq!(
            queue1_after_dequeue.size(),
            1,
            "dequeue操作後の新しいキューのサイズは減少しているべき"
        );
        assert_eq!(
            queue1_after_dequeue.peek().unwrap(),
            2,
            "dequeue操作後の新しいキューの先頭要素は次の要素であるべき"
        );
    }
