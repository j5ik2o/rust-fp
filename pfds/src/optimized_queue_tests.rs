use crate::{OptimizedQueue, Queue};
...
mod tests {
    ...
    #[test]
    fn test_functor() {
        let queue = OptimizedQueue::empty().enqueue(1).enqueue(2).enqueue(3);
    ...
    #[test]
    fn test_pure() {
        let queue = OptimizedQueue::<i32>::pure(42);
    ...
    fn test_apply() {
        ...
        let functions = OptimizedQueue::empty()
    ...
    fn test_applicative() {
        ...
        let queue = OptimizedQueue::<i32>::pure(5);
        // Use an enum to represent functions
        #[derive(Clone)]
        enum IntFunction {
            Triple,
        ...
        let functions = OptimizedQueue::pure(IntFunction::Triple);
    ...
    fn test_bind() {
        ...
        let result_queue = queue.bind(|x| {
            let mut q = OptimizedQueue::empty();
            q = q.enqueue(x * 2);
            q.enqueue(x + 10)
        });
    ...
    fn test_monad() {
        ...
        let result_queue = queue.bind(|x| OptimizedQueue::pure(x * 3));
    ...
    fn test_foldable() {
        ...
        let queue2 = OptimizedQueue::empty().enqueue(1).enqueue(2).enqueue(3);

        let product_left = queue2.fold_left(1, |acc, x| acc * x);
        assert_eq!(product_left, 6);

        // Test more complex fold_right
        let queue3 = OptimizedQueue::empty().enqueue(1).enqueue(2).enqueue(3);
